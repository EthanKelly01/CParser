...
namespace Parser {
	...
	//The type of each primitive variable (int, char, float, double, bool, etc)
	struct primitive : public datatypes {
		std::vector<std::string> modifiers;
		std::string type, name;

		primitive(std::string t_type, std::string t_name, std::vector<std::string> t_modifiers = {}) : type(t_type), name(t_name), modifiers(t_modifiers) { type = "primitive"; };
	...
	//derived types of data (arrays, pointers, etc)
	struct complex : public datatypes {
		primitive prim;
		std::string modifier;

		complex(primitive t_prim, std::string t_modifier) : prim(t_prim), modifier(t_modifier) { type = "complex"; };
	...
	struct function : public datatypes {
		...
		function(datatypes* t_returnType, std::string t_name, std::vector<datatypes*> t_params = {}) : returnType(t_returnType), name(t_name), params(t_params) { type = "function"; };
	...
	struct classes : public datatypes {
		...
		classes(
			...
			m_public(t_public), m_private(t_private), m_protected(t_protected) { type = "classes"; };
	...
	struct namespaces : public datatypes {
		...
		namespaces(std::string t_name, std::vector<datatypes*> t_members = {}) : name(t_name), members(t_members) { type = "namespaces"; };
